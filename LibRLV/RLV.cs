...
namespace LibRLV
{
    public class RLV
    {
        ...
        private void RemoveRule(RLVRule rule)
        {
            if (!CurrentRules.TryGetValue(rule.Behavior, out var rules))
            {
                return;
            }

            rules.RemoveAll(n => n.Args.SequenceEqual(rule.Args));
            RulesUpdated?.Invoke(this, new RulesUpdatedEventArgs()
            {
                IsDeleted = true,
                IsNewRule = false,
        ...
        private bool ProcessForceCommand(RLVMessage command)
        {
        ...
        private bool ProcessRule(RLVMessage command)
        {
            ...
            {
                return ProcessForceCommand(command);
            }
